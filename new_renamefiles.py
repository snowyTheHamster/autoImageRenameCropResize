    ...
    for _, _, files in os.walk(TMP_DIR):
        ...
        for filename in files:
            ...
            for row in csv_f: # per each row in csv file
                for y in range(no_of_files): # repeat below loop x number of times per csv row
                    y = y + 1

                    if y == 1:
                        new_file = f'{row[0]}_XL{file_ext}' # create desired filename from csv
                    elif y == 2:
                        new_file = f'{row[0]}_01_XL{file_ext}' # create desired filename from csv
...
layout = [
    ...
    [sg.Text('No. per file:'), sg.InputText(key='_FILE_VARIATIONS_', size=(5,1))],
...
try:
    while True:
        ...
        if event == '_PROCESS_':
            ...
            no_of_files = int(values['_FILE_VARIATIONS_'])
            ...
            if INPUT_DIR == '' or OUTPUT_DIR == '':
                ...
                print('must speficy no. of variations per file')
            else:
                renameitup()
